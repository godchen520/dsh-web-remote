        ...
        if (/^\/监听$/.test(t) || /^\/监控$/.test(t)) {
          ...
          if (weixinMonitorMode) {
            ...
            weixinMonitorTimer = setInterval(async function () {
              if (!weixinMonitorMode || !agents) { if (weixinMonitorTimer) { clearInterval(weixinMonitorTimer); weixinMonitorTimer = null; } return; }
              try {
                var agentList = [];
                      ...
                      if (evts[j].type === 'assistant/message') {
                        ...
                        if (parts.length > 0) {
                          ...
                          if (!sessTitle && sessionQuery) {
                            try {
                              var _tr = await sessionQuery.readTitle(aid);
                              sessTitle = (typeof _tr === 'string') ? _tr : (_tr && _tr.title) ? _tr.title : '';
                            } catch (e2) {}
                          }
