...
export function apply(ctx, rawConfig) {
  ...
  // 重启 DSH 待执行标记
  let weixinPendingRestart = false;
          ...
          for (const msg of res.msgs) {
            if (msg.message_type === 1 && msg.item_list && msg.item_list.length > 0) {
              ...
              if (textItem && msg.from_user_id) {
                ...
                // 重启 DSH：回复已发出，执行重启
                if (weixinPendingRestart) {
                  weixinPendingRestart = false;
                  console.log('[dsh-weixin] restarting DSH...');
                  try {
                    const restartConfig = JSON.stringify({
                      pid: process.pid,
                      command: process.argv[0] || 'node',
                      args: process.argv.slice(1),
                      cwd: process.cwd()
                    });
                    const configPath = path.join(toolsDir, 'restart-config.json');
                    fs.writeFileSync(configPath, restartConfig);
                    const restartScript = path.join(__dirname, 'restart.js');
                    const child = spawn(process.execPath, [restartScript, configPath], {
                      detached: true, stdio: 'ignore', windowsHide: true
                    });
                    child.unref();
                    console.log('[dsh-weixin] restart helper spawned');
                  } catch (e) {
                    console.error('[dsh-weixin] restart spawn failed:', e.message);
                  }
                }
              }
            }
  ...
  const snapshot = () => ({ running: state.running, url: state.url, token: state.token, port: state.port, httpsPort: state.httpsPort, ips: state.ips, qq: state.qq, weixin: weixinState.status, error: state.error, lanOpen: config.lanOpen });
  ...
  if (webServer) {
    ...
    async function handleWeixinCommand(text) {
      ...
      if (/^\//.test(t)) {
        ...
        // /重启DSH
        if (/^\/重启DSH$/.test(t) || /^\/重启$/.test(t)) {
          weixinPendingRestart = true;
          return '正在重启 DSH，约 10 秒后自动恢复…';
        }
        // /切换模型（多步交互）
        if (/^\/切换模型/.test(t)) {
