        ...
        if (ch.id === 'feishu') {
          ...
          confirmBtn.addEventListener('click', function () {
            ...
            fetch('/remote/feishu/config', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ appId: idVal, appSecret: secVal }) }).then(function (r) { return r.json(); }).then(function (d) {
              if (d.ok && d.connected) { if (st2) { st2.textContent = ''; st2.appendChild(el('span', '', '\u5df2\u8fde\u63a5')); st2.querySelector('span').previousElementSibling.style.background = '#22c55e'; } qrZone.textContent = ''; qrZone.appendChild(el('div', '', '\u98de\u4e66\u673a\u5668\u4eba\u5df2\u8fde\u63a5\uff0c\u53ef\u5728\u98de\u4e66\u4e2d\u53d1\u6d88\u606f\u63a7\u5236 DSH')); }
  ...
  if (webServer) {
    ...
    const feishuConfigHandler = async (req, res) => {
      ...
      try {
        ...
        if (v.ok) { feishuToken = null; await feishuEnsureToken(); console.log('[feishu] \u51ed\u8bc1\u9a8c\u8bc1\u901a\u8fc7'); res.writeHead(200, { 'content-type': 'application/json; charset=utf-8' }); res.end(JSON.stringify({ ok: true, connected: true })); }
