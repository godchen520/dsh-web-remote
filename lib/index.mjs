    ...
    if (currentBotChannel) {
      ...
      if (ch) {
        ...
        if (ch.id === 'feishu') {
          // 飞书：先查状态，再决定显示什么
          qrZone.textContent = '';
          var feishuCheckDiv = el('div', '', '\u68c0\u67e5\u4e2d\u2026');
          qrZone.appendChild(feishuCheckDiv);
          fetch('/remote/feishu/status').then(function (r) { return r.json(); }).then(function (fs) {
            qrZone.textContent = '';
            if (fs.connected) {
              // 已连接
              if (st2) { st2.textContent = ''; st2.appendChild(el('span', '', '\u5df2\u8fde\u63a5')); var dot3 = st2.querySelector('.webrm-bot-dot'); if (dot3) dot3.style.background = '#22c55e'; }
              var connInfo = el('div', '', '');
              connInfo.style.cssText = 'padding:4px 0;font-size:13px;color:var(--dsw-alias-label-primary,#1d1d1f)';
              connInfo.appendChild(el('div', 'webrm-label2', '\u98de\u4e66\u673a\u5668\u4eba\u5df2\u8fde\u63a5'));
              var appIdDisp = el('div', '', 'App ID: ' + (fs.appId || ''));
              appIdDisp.style.cssText = 'font-size:12px;color:var(--dsw-alias-label-secondary,#888);margin:4px 0';
              connInfo.appendChild(appIdDisp);
              qrZone.appendChild(connInfo);
            } else if (fs.configured) {
              // 已配置但未连接（WebSocket 失败）
              if (st2) { st2.textContent = ''; st2.appendChild(el('span', '', '\u5df2\u914d\u7f6e')); }
              var reconnBtn = el('button', 'webrm-btn webrm-btn-primary', '\u91cd\u65b0\u8fde\u63a5');
              reconnBtn.type = 'button';
              reconnBtn.style.cssText = 'width:100%;margin-top:8px';
              reconnBtn.addEventListener('click', function () {
                if (st2) { st2.textContent = ''; st2.appendChild(el('span', '', '\u8fde\u63a5\u4e2d\u2026')); }
                fetch('/remote/feishu/config', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ appId: fs.appId, appSecret: ' ' }) }).then(function (r) { return r.json(); }).then(function (d) {
                  if (d.connected) { if (st2) { st2.textContent = ''; st2.appendChild(el('span', '', '\u5df2\u8fde\u63a5')); var dot4 = st2.querySelector('.webrm-bot-dot'); if (dot4) dot4.style.background = '#22c55e'; } }
                  else { if (st2) { st2.textContent = ''; st2.appendChild(el('span', '', '\u91cd\u8fde\u5931\u8d25: ' + (d.error || '\u672a\u77e5\u9519\u8bef'))); st2.querySelector('span').style.color = '#ef4444'; } }
                }).catch(function () { if (st2) { st2.textContent = ''; st2.appendChild(el('span', '', '\u8bf7\u6c42\u5931\u8d25')); } });
              });
              qrZone.appendChild(reconnBtn);
            } else {
              // 未配置：显示输入表单
              var formDiv = el('div', '', '');
              formDiv.style.cssText = 'padding:4px 0;font-size:13px;color:var(--dsw-alias-label-primary,#1d1d1f)';
              formDiv.appendChild(el('div', 'webrm-label2', '\u8bf7\u586b\u5199\u98de\u4e66\u5f00\u653e\u5e73\u53f0\u7684 App ID \u548c App Secret'));
              var inId = el('input', '', '');
              inId.placeholder = 'App ID (cli_xxx)';
              inId.style.cssText = 'width:100%;box-sizing:border-box;padding:7px 10px;margin:6px 0;background:var(--dsw-alias-interactive-bg-hover,rgba(120,120,128,.08));border:1px solid var(--dsw-alias-border-l2,rgba(0,0,0,.15));border-radius:8px;font-size:13px;outline:none;color:var(--dsw-alias-label-primary,#1d1d1f)';
              formDiv.appendChild(inId);
              var inSec = el('input', '', '');
              inSec.type = 'password';
              inSec.placeholder = 'App Secret';
              inSec.style.cssText = inId.style.cssText;
              formDiv.appendChild(inSec);
              var confirmBtn = el('button', 'webrm-btn webrm-btn-primary', '\u786e\u8ba4\u5e76\u9a8c\u8bc1');
              confirmBtn.type = 'button';
              confirmBtn.style.cssText = 'margin-top:8px;width:100%';
              confirmBtn.addEventListener('click', function () {
                var idVal = inId.value.trim();
                var secVal = inSec.value.trim();
                if (!idVal || !secVal) { var stErr = document.getElementById('webrm-bot-strow'); if (stErr) { stErr.appendChild(el('span', '', ' \u8bf7\u586b\u5199\u5168\u90e8\u51ed\u8bc1')); stErr.querySelector('span:last-child').style.color = '#ef4444'; } return; }
                var st2 = document.getElementById('webrm-bot-strow');
                if (st2) { st2.textContent = ''; st2.appendChild(el('span', '', '\u9a8c\u8bc1\u4e2d\u2026')); }
                fetch('/remote/feishu/config', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ appId: idVal, appSecret: secVal }) }).then(function (r) { return r.json(); }).then(function (d) {
                  if (d.ok && d.connected) { if (st2) { st2.textContent = ''; st2.appendChild(el('span', '', '\u5df2\u8fde\u63a5')); var dot2 = st2.querySelector('.webrm-bot-dot'); if (dot2) dot2.style.background = '#22c55e'; } qrZone.textContent = ''; qrZone.appendChild(el('div', '', '\u98de\u4e66\u673a\u5668\u4eba\u5df2\u8fde\u63a5\uff0c\u53ef\u5728\u98de\u4e66\u4e2d\u53d1\u6d88\u606f\u63a7\u5236 DSH')); }
              ...
              var guideLink = el('a', '', '\u2192 \u6253\u5f00\u98de\u4e66\u5f00\u653e\u5e73\u53f0');
              guideLink.href = 'https://open.feishu.cn';
              guideLink.target = '_blank';
              guideLink.style.cssText = 'font-size:12px;color:var(--dsw-alias-label-secondary,#888);display:inline-block;margin-top:10px';
              formDiv.appendChild(confirmBtn);
              formDiv.appendChild(guideLink);
              qrZone.appendChild(formDiv);
            }
          }).catch(function () {
            qrZone.textContent = '';
            qrZone.appendChild(el('div', '', '\u68c0\u67e5\u72b6\u6001\u5931\u8d25'));
          });
        } else {
...
export function apply(ctx, rawConfig) {
  ...
  const snapshot = () => ({ running: state.running, url: state.url, token: state.token, port: state.port, httpsPort: state.httpsPort, ips: state.ips, qq: state.qq, weixin: weixinState.status, error: state.error, lanOpen: config.lanOpen, customPublicUrl: customPublicUrl || null });
  ...
  if (webServer) {
    ...
    async function feishuStartWS() {
      ...
      try {
        ...
        const dispatcher = new EventDispatcher({}).register({
          'im.message.receive_v1': async (data) => {
            try {
              ...
              const senderId = data.sender && data.sender.sender_id && data.sender.sender_id.open_id;
              console.log('[feishu] received:', text.slice(0, 100), 'from', chatId);
              // 复用现有 handleWeixinCommand 逻辑（但简化版，只处理命令）
        ...
        feishuWSClient.start(dispatcher);
    ...
    async function feishuHandleCommand(text, chatId) {
      ...
      if (!t) return null;
      // 简化命令：直接返回帮助信息
      if (/^\/?(help|帮助)$/i.test(t)) return '\u98de\u4e66\u673a\u5668\u4eba\u5df2\u5728\u7ebf\uff0c\u652f\u6301\u547d\u4ee4\uff1a\n/\u94fe\u63a5 - \u83b7\u53d6 DSH \u8fdc\u7a0b\u94fe\u63a5\n/\u72b6\u6001 - \u67e5\u770b DSH \u72b6\u6001\n/\u5e2e\u52a9 - \u663e\u793a\u547d\u4ee4\u5217\u8868';
      if (/^\/?(\u94fe\u63a5|\u83b7\u53d6\u94fe\u63a5|link)$/i.test(t)) {
        const info = snapshot();
        if (info.url && info.token) return 'DSH \u8fdc\u7a0b\u8bbf\u95ee\u94fe\u63a5\uff1a\n' + info.url + '/?token=' + info.token;
        return '\u8fdc\u7a0b\u901a\u9053\u672a\u542f\u52a8\uff0c\u8bf7\u5728 DSH \u9762\u677f\u542f\u52a8\u3002';
      }
      if (/^\/?(\u72b6\u6001|status)$/i.test(t)) {
        const info = snapshot();
        return 'DSH \u72b6\u6001\uff1a' + (info.running ? '\u8fd0\u884c\u4e2d' : '\u5df2\u505c\u6b62') + '\uff0c\u8fde\u63a5\u6570: ' + (info.ips ? info.ips.length : 0);
      }
