                ...
                if (evts[j].type === 'assistant/message') {
                  ...
                  if (parts.length > 0) {
                    ...
                    if (weixinMonitorMode && weixinState.botToken && weixinState.lastFromUserId) {
                      var ctxAgeMin = weixinLastContextAt ? Math.round((Date.now() - weixinLastContextAt) / 60000) : Infinity;
                      if (ctxAgeMin > 25) {
                        // 凭据过期：先试空凭据主动发送（若 iLink 支持则无需积压）；API 拒绝再入队
                        weixinSendMsg(weixinState.botToken, weixinState.lastFromUserId, text, '').then(function () {
                          console.log('[dsh-weixin] 空凭据发送已提交（凭据 ' + (ctxAgeMin === Infinity ? '未知' : ctxAgeMin) + ' 分钟未刷新）——请确认微信实际收到');
                        }).catch(function (e) {
                          console.log('[dsh-weixin] 空凭据被拒（' + e.message + '），通知入队');
