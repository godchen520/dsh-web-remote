...
const INJECT_SCRIPT = `(function () {
  ...
  function botChannelStatus(id, info) {
    ...
    if (id === 'feishu') return (info && info.feishu === 'connected') ? '\u5df2\u8fde\u63a5' : '\u672a\u914d\u7f6e';
    return '\u672a\u63a5\u5165';
  }
...
export function apply(ctx, rawConfig) {
  ...
  const snapshot = () => ({ running: state.running, url: state.url, token: state.token, port: state.port, httpsPort: state.httpsPort, ips: state.ips, qq: state.qq, weixin: weixinState.status, error: state.error, lanOpen: config.lanOpen, customPublicUrl: customPublicUrl || null, feishu: (typeof feishuToken !== 'undefined' && feishuToken && Date.now() < feishuToken.expireAt) ? 'connected' : null });
