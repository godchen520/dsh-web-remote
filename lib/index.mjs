  ...
  if (webServer) {
    ...
    async function feishuHandleCommand(text, chatId) {
      ...
      if (/^\/?(\u76d1\u542c|\u76d1\u63a7)$/i.test(t)) {
        ...
        try {
          ...
          var mon = {};
          try { mon = JSON.parse(fs.readFileSync(monFile, 'utf-8')) || {}; } catch (e) {}
          if (!mon.weixin) mon.weixin = {};
          mon.feishu = { enabled: feishuMonitorMode };
          fs.writeFileSync(monFile, JSON.stringify(mon), 'utf-8');
        } catch (e) { console.error('[feishu] monitor save failed:', e.message); }
        if (feishuMonitorMode) { try { startMonitor(); } catch (e) {} return '\u98de\u4e66\u76d1\u542c\u5df2\u5f00\u542f\uff0c\u518d\u6b21\u53d1\u9001 /\u76d1\u542c \u53ef\u5173\u95ed'; }
        else {
          if (!weixinMonitorMode) { try { stopMonitor(); } catch (e) {} }
          return '\u98de\u4e66\u76d1\u542c\u5df2\u5173\u95ed';
        }
      }
      ...
      if (/^\//.test(t)) {
        ...
        if (/^\/监听$/.test(t) || /^\/监控$/.test(t)) {
          ...
          try {
            ...
            var mon = {};
            try { mon = JSON.parse(fs.readFileSync(monFile, 'utf-8')) || {}; } catch (e) {}
            if (!mon.feishu) mon.feishu = {};
            mon.weixin = { enabled: weixinMonitorMode, userId: weixinState.lastFromUserId || '', contextToken: weixinState.lastContextToken || '' };
            fs.writeFileSync(monFile, JSON.stringify(mon), 'utf-8');
          } catch (e) { console.error('[dsh-weixin] monitor save failed:', e.message); }
      ...
      if (savedToken) {
        ...
        setTimeout(function () {
          try {
            var monFile = path.join(toolsDir, 'monitor-mode.json');
            var md = {};
            try { md = JSON.parse(fs.readFileSync(monFile, 'utf-8')) || {}; } catch (e) {}
            // 兼容旧格式 {enabled, userId, contextToken}
            if (md.enabled && !md.weixin) md.weixin = { enabled: true, userId: md.userId || '', contextToken: md.contextToken || '' };
    ...
    setTimeout(function () {
      try {
        ...
        var md = {};
        try { md = JSON.parse(fs.readFileSync(monFile, 'utf-8')) || {}; } catch (e) {}
        // 兼容旧格式 {enabled, userId, contextToken}（视为微信）
        if (md.enabled && !md.weixin) md.weixin = { enabled: true, userId: md.userId || '', contextToken: md.contextToken || '' };
        ...
        if (!feishuLastChatId) {
          try {
            ...
            if (savedChat && savedChat.chatId) feishuLastChatId = savedChat.chatId;
          } catch (e) {}
        }
