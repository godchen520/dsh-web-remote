  ...
  if (webServer) {
    ...
    setTimeout(function () {
      try {
        ...
        if (!weixinState.lastFeishuChatId) {
          try {
            var savedChat = JSON.parse(fs.readFileSync(path.join(toolsDir, 'feishu-last-chat.json'), 'utf-8'));
            if (savedChat && savedChat.chatId) weixinState.lastFeishuChatId = savedChat.chatId;
          } catch (e) {}
        }
        // 发飞书启动通知
        if (weixinState.lastFeishuChatId) {
