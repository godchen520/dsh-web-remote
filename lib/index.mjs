...
//   tunnelMode      公网隧道模式 quick|custom|none  默认 quick
//                  quick ：自动起 Cloudflare Quick Tunnel（现状，零配置即开箱即用）
//                  custom：只起反向代理，不 spawn 隧道——复用已有稳定隧道/域名
//                          （配合 publicUrl 显示你已有的公网地址，如 named tunnel + Access）
//                  none  ：仅局域网直连，不起任何公网隧道
//   publicUrl       自定义公网地址（仅 tunnelMode=custom 时用于面板展示）默认 ''
//   requireToken    是否需要 token 验证  默认 true；false 则彻底关闭 token/cookie 校验
//                  （务必配合 Cloudflare Access 等边缘访问控制使用，否则公网等于裸奔）
...
export function createProxyServer(options) {
  const { targetPort, pfxPath, pfxPass, lanOpen = true, requireToken = true } = options;
  ...
  function isAuthed(req) {
    // requireToken=false：彻底关闭 token/cookie 校验（配合 Cloudflare Access 等
    // 边缘访问控制使用；否则公网等于裸奔）。
...
export function createQQServer(options) {
  ...
  function handlePrivate(evt, send) {
    ...
    if (/远程|链接|网址|token|地址/.test(message)) {
      fetchInfo().then((info) => {
        const pub = (info && (info.url || info.publicUrl)) ? (info.url || info.publicUrl) : null;
        const needToken = !!(info && info.requireToken);
        if (pub && (info.token || !needToken)) {
          reply('DSH 远程访问链接：' + pub + (needToken ? '/?token=' + info.token : '') + '\n手机浏览器打开即可使用。插件重启后链接会变化，可再次发送本指令获取。');
        } else {
...
const INJECT_SCRIPT = `(function () {
  ...
  function syncActionButtons(info) {
    ...
    // custom / none 模式没有 quick tunnel 的「换新链接」可换
    var rf = document.getElementById('webrm-refresh');
    if (rf) rf.style.display = (!info || info.tunnelMode === 'quick') ? '' : 'none';
  }
  // 机器人通道定义
  var BOT_CHANNELS = [
  ...
  function renderStatus(panel, info, hint) {
    ...
    box.textContent = '';
    var urls = [];
    // requireToken=false 时链接不带 ?token=（token 校验已关闭，携带也无意义）
    var withToken = function (info, base) { return (info && info.requireToken && info.token) ? base + '/?token=' + info.token : base; };
    if (currentTab === 'lan' && info && info.ips && info.ips.length) {
      urls = info.ips.map(function (ip) { return { label: '局域网直连 ' + ip + '（点击复制）', url: withToken(info, (info.httpsPort ? 'https://' : 'http://') + ip + ':' + (info.httpsPort || info.port)) }; });
    } else if (info && info.tunnelMode === 'custom' && info.publicUrl) {
      // custom：复用已有隧道，公网地址由 publicUrl 提供（token 作为第二道鉴权，可按需关闭）
      urls = [{ label: '公网访问链接（点击复制）', url: withToken(info, info.publicUrl) }];
    } else if (info && info.url) {
      urls = [{ label: '公网访问链接（点击复制）', url: withToken(info, info.url) }];
    }
    if (urls.length === 0) {
      if (info && info.running && currentTab === 'public' && info.tunnelMode !== 'none') {
        box.appendChild(el('div', '', '隧道已断开：请点「停止」后重新「启动」'));
      } else if (info && info.running && currentTab === 'public' && info.tunnelMode === 'custom' && !info.publicUrl) {
        box.appendChild(el('div', '', 'custom 模式：请在插件配置里设置 publicUrl 公网地址'));
      } else if (info && info.running && currentTab === 'public') {
        box.appendChild(el('div', '', '仅局域网模式：未配置公网隧道'));
      } else {
    ...
    var qrTarget = currentTab === 'lan' && info && info.ips && info.ips.length ? withToken(info, (info.httpsPort ? 'https://' : 'http://') + info.ips[0] + ':' + (info.httpsPort || info.port)) : (info && info.tunnelMode === 'custom' && info.publicUrl ? withToken(info, info.publicUrl) : (info && info.url ? withToken(info, info.url) : null));
    if (qrTarget) {
      var qi = 0;
      var q = el('img', '', '');
      q.id = 'webrm-qr';
  ...
  function openPanel() {
    ...
    function refresh() {
      ...
      fetchInfo().then(function (info) {
        ...
        if (info && info.running && currentTab === 'public' && info.tunnelMode === 'quick' && !info.url && retryCount < 3) {
          retryCount += 1;
...
export function apply(ctx, rawConfig) {
  ...
  const config = {
    ...
    tunnelMode: cfg.tunnelMode ?? 'quick',
    publicUrl: cfg.publicUrl ?? '',
    requireToken: cfg.requireToken ?? true,
  };
  ...
  const start = async () => {
    ...
    try {
      ...
      proxy = createProxyServer({ targetPort: config.targetPort, pfxPath: config.pfxPath, pfxPass: config.pfxPass, lanOpen: config.lanOpen, requireToken: config.requireToken });
      ...
      // 2. cloudflared 隧道（仅 quick 模式自动起 Quick Tunnel）
      if (config.tunnelMode === 'quick') {
        ...
        // custom / none：不 spawn 隧道。custom 复用已有隧道（公网地址由 publicUrl 提供）。
        // 代理已就绪，公网入口由用户自己的隧道/域名承担。
        state.url = null;
      }
  ...
  const snapshot = () => ({ running: state.running, url: state.url, token: state.token, port: state.port, httpsPort: state.httpsPort, ips: state.ips, qq: state.qq, weixin: weixinState.status, error: state.error, lanOpen: config.lanOpen, tunnelMode: config.tunnelMode, publicUrl: config.publicUrl, requireToken: config.requireToken });
    ...
    async function handleWeixinCommand(text) {
      ...
      if (/^\//.test(t)) {
        ...
        if (/^\/链接$/.test(t) || /^\/公网链接$/.test(t) || /^\/获取公网链接$/.test(t)) {
          ...
          // custom：复用已有隧道，公网地址由 publicUrl 提供
          if (config.tunnelMode === 'custom' && config.publicUrl && (state.token || !config.requireToken)) {
            weixinFollowup = '[如果用外部浏览器，请直接复制链接，从微信内部浏览器跳转，会丢失验证信息导致验证失败]';
            return '[手机浏览器建议页面缩放比例调整为50%-75%，以获得更好的显示效果]\n公网链接：\n' + config.publicUrl + (config.requireToken ? '/?token=' + state.token : '');
          }
          if (config.tunnelMode === 'none') {
            return '当前为「仅局域网」模式（tunnelMode=none），没有公网链接；请改用局域网地址或切换 tunnelMode=custom/quick';
          }
          if (state.url && (state.token || !config.requireToken)) {
            weixinFollowup = '[如果用外部浏览器，请直接复制链接，从微信内部浏览器跳转，会丢失验证信息导致验证失败]';
            return '[手机浏览器建议页面缩放比例调整为50%-75%，以获得更好的显示效果]\n公网链接：\n' + state.url + (config.requireToken ? '/?token=' + state.token : '');
          }
